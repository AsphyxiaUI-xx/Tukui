...
if not C["datatext"].hit == nil or C["datatext"].hit > 0 then
	...
	if int < 0 then
			if attackpwr > spellpwr and select(2, UnitClass("Player")) ~= "HUNTER" then
				Text:SetText("Hit: "..hexa..format("%.2f", GetCombatRatingBonus(6)).."%")
			elseif select(2, UnitClass("Player")) == "HUNTER" then
				Text:SetText("Hit: "..hexa..format("%.2f", GetCombatRatingBonus(7)).."%")
			else
				Text:SetText("Hit: "..hexa..format("%.2f", GetCombatRatingBonus(8)).."%")
			end
			int = 1
		end
	end
