...
if C["datatext"].avd and C["datatext"].avd > 0 then
	...
	local function Update(self)
		...
		if leveldifference >= 0 then
			...
			Text:SetText(L.datatext_playeravd..hexa..format("%.2f", avoidance))
		else
	...
	Stat:SetScript("OnEnter", function(self)
		if not InCombatLockdown() then
			...
			if targetlv > 1 then
				GameTooltip:AddDoubleLine(L.datatext_avoidancebreakdown.." ("..LEVEL_ABBR.." "..targetlv..")")
			elseif targetlv == -1 then
				GameTooltip:AddDoubleLine(L.datatext_avoidancebreakdown.." ("..BOSS..")")
			else
				GameTooltip:AddDoubleLine(L.datatext_avoidancebreakdown.." ("..LEVEL_ABBR.." "..targetlv..")")
			end
			GameTooltip:AddDoubleLine(COMBAT_TEXT_MISS,format("%.2f",MissChance) .. "%",1,1,1,  1,1,1)
			GameTooltip:AddDoubleLine(COMBAT_TEXT_DODGE,format("%.2f",dodge) .. "%",1,1,1,  1,1,1)
			GameTooltip:AddDoubleLine(COMBAT_TEXT_PARRY,format("%.2f",parry) .. "%",1,1,1,  1,1,1)
			GameTooltip:AddDoubleLine(COMBAT_TEXT_BLOCK,format("%.2f",block) .. "%",1,1,1,  1,1,1)
			GameTooltip:Show()
		end
	end)
