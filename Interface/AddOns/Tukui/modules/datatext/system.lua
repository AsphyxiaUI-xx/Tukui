...
local kiloByteString = "%d"..hexa.." kb"
local megaByteString = "%.2f"..hexa.." mb"
...
Stat:SetScript("OnEnter", function(self)
	if not InCombatLockdown() then
		...
		if bandwidth ~= 0 then
			GameTooltip:AddDoubleLine(L.datatext_bandwidth , string.format(bandwidthString, bandwidth),0, 0.6, 1, 1, 1, 1)
			GameTooltip:AddDoubleLine(L.datatext_download , string.format(percentageString, GetDownloadedPercentage() *100),0, 0.6, 1, 1, 1, 1)
			GameTooltip:AddLine(" ")
		end
		local totalMemory = UpdateMemory()
		GameTooltip:AddDoubleLine(L.datatext_totalmemusage, formatMem(totalMemory),0, 0.6, 1, 1, 1, 1)
		GameTooltip:AddLine(" ")
		for i = 1, #memoryTable do
