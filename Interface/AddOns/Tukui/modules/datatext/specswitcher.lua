...
if C["datatext"].specswitcher and C["datatext"].specswitcher > 0 then
	...
	local function Update(self, t)
		if not GetPrimaryTalentTree() then
			Text:SetText(hexa..L.datatext_notalents..hexb) 
		return end
		int = int - t
		if int < 0 then
			...
			Text:SetText(select(2,GetTalentTabInfo(majorTree))..": "..hexa..tree1num.."/"..tree2num.."/"..tree3num)
		end
	end
	...
	local function OnEvent(self, event, ...)
		...
		self:SetScript("OnEnter", function(self)
			if not InCombatLockdown() then
				...
				if(not GetPrimaryTalentTree()) then
					GameTooltip:AddLine(hexa..L.datatext_notalents)
				elseif(hs) then
					GameTooltip:AddLine(hexa..(c == 1 and "* " or "  ") .. "|r" .. select(2,GetTalentTabInfo(majorTree1))..": "..hexa..group1tree1.."/"..group1tree2.."/"..group1tree3,1,1,1)
					GameTooltip:AddLine(hexa..(c == 2 and "* " or "  ") .. "|r" .. select(2,GetTalentTabInfo(majorTree2))..": "..hexa..group2tree1.."/"..group2tree2.."/"..group2tree3,1,1,1)
				else
					GameTooltip:AddLine(select(2,GetTalentTabInfo(majorTree1))..": "..hexa..group1tree1.."/"..group1tree2.."/"..group1tree3,1,1,1)
				end
