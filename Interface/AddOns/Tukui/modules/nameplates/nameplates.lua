...
local PlateBlacklist = {
	--Shaman Totems
	["Тотем элементаля земли"] = true,
	["Тотем элементаля огня"] = true,
	["Тотем сопротивления силам стихий"] = true,
	...
	["Тотем гнева"] = true,
	["Тотем гнева воздуха"] = true,
	["Тотем оков земли"] = true,
	["Тотем заземления"] = true,
	["Тотем трепета"] = true,
	["Тотем безмятежного разума"] = true,
...
local function UpdateObjects(frame)
	...
	if C["nameplate"].showhealth == true then
		frame.hp.level:SetPoint("RIGHT", frame.hp, "RIGHT", 0, 1)
	else
...
local function SkinObjects(frame)
	...
	if C["nameplate"].showhealth == true then
		...
		hp.value:SetPoint("CENTER", hp, 1, 1)
